...
<div class="container mt-3">
    ...
    <table id="subcategoryTable" class="display" style="width:100%">
        ...
        <tbody>
           ...
           <tr>
               <td>{{ $loop->index + 1 }}</td>
               <td>{{ $row->category->name }} ---> {{ $row->subcategory_name }}</td>
               ...
               <td>
                    <a href="{{ route('admin.subcategory.edit', $row->id) }}" class="btn btn-outline-primary btn-sm">
                    <i class="fas fa-edit"></i></a>

                    <form action="{{ route('admin.subcategory.destroy', $row->id) }}" id="delete_form" method="POST">
                        @csrf

                        @method('DELETE')

                        <button type="submit" id="delete_subcategory" class="btn btn-outline-danger btn-sm mt-2"><i class="fas fa-trash"></i></button>
                    </form>
...
@section('js')
    ...
    <script>
        function update_published(el){
            ...
            $.post('{{ route('admin.subcategory.inactive') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                //console.log(data);
                if(data == 1){
                //    Swal.fire(
                //     'Good job!',
